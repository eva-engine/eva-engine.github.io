...
下面以 [@eva/plugin-a11y](a11yPlugin) 插件为例，对 Eva.js 插件开发做一个详细的介绍。
...
如果想实现这些功能，就要去在当前组件下拿到别的组件去实现功能了，如果想触发点击事件，需要判断 [Event 组件](interactionEvent)是否安装，如果安装的话，可以根据 Event 上绑定的事件，触发对应的事件。如果想获取宽高位置的话，可以获取游戏对象的 [Transform 组件](transformComponent)
...
通过 Component 和 System 的结合，我们可以实现各种各样的通用插件，在日常开发中，我们仅需要 [CustomComponent](customComponent) 提供的能力开发游戏逻辑即可。
